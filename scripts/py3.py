...
class cmd_vel:
    ...
    def __init__(self):
        ...
        self.pattern1=re.compile(r'(?P<router>\u5706|\u6B63\u65B9\u5F62|\u957F\u65B9\u5F62|\u77E9\u5F62|\u524D\u8FDB|\u5411\u524D|\u52A0\u901F|\u51CF\u901F|\u5DE6\u8F6C|\u53F3\u8F6C|\u540E\u9000|\u505C\u6B62|\u7ACB\u5B9A|\u7EE7\u7EED|\u6062\u590D)')
        # self.pattern2=re.compile(r'(?P<way>半径|直径|边长|前进|向前)[^\d|一|二|三|四|五|六|七|八|九|十](?P<length>\d+|一|二|三|四|五|六|七|八|九|十).*(?P<direction>正|顺|逆)*.*(?P<router>圆|正方形)')
        self.pattern2=re.compile(r'(?P<way>\u534A\u5F84|\u76F4\u5F84|\u8FB9\u957F|\u524D\u8FDB|\u5411\u524D)\D*(?P<length>\d+|\u4E00|\u4E8C|\u4E09|\u56DB|\u4E94|\u516D|\u4E03|\u516B|\u4E5D|\u5341)[^\u6B63|\u987A|\u9006]*(?P<direction>\u6B63(?:.[^\u5F62])|\u987A|\u9006)?')
        # 长-宽 类型长方形匹配
        self.pattern3=re.compile(r'(?P<way_length>\u957F)\D*(?P<length>\d+|\u4E00|\u4E8C|\u4E09|\u56DB|\u4E94|\u516D|\u4E03|\u516B|\u4E5D|\u5341).*(?P<way_wide>\u5BBD)\D*(?P<wide>\d+|\u4E00|\u4E8C|\u4E09|\u56DB|\u4E94|\u516D|\u4E03|\u516B|\u4E5D|\u5341)[^\u6B63|\u987A|\u9006]*(?P<direction>\u6B63|\u987A|\u9006)?')
        # 宽-长 类型长方形匹配
        self.pattern4=re.compile(r'(?P<way_wide>\u5BBD)\D*(?P<wide>\d+|\u4E00|\u4E8C|\u4E09|\u56DB|\u4E94|\u516D|\u4E03|\u516B|\u4E5D|\u5341).*(?P<way_length>\u957F)\D*(?P<length>\d+|\u4E00|\u4E8C|\u4E09|\u56DB|\u4E94|\u516D|\u4E03|\u516B|\u4E5D|\u5341)[^\u6B63|\u987A|\u9006]*(?P<direction>\u6B63|\u987A|\u9006)?')
        # 循环指令匹配pattern5=re.compile(r'(?P<notLoop>一个|走个)')
        self.pattern5=re.compile(r'(?P<notLoop>\u8D70\u4E2A|\u4E00\u4E2A)')
    ...
    def msg_Twist(self,value,type,direction=1):
        ...
        if type==1:
            ...
            temp['msg_Buff'].linear.x=speed_temp
        elif type==2:
